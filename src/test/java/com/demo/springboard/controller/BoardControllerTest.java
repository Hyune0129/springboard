...
public class BoardControllerTest {
        ...
        void getChangePostPageTest() throws Exception {
                BoardDTO board = BoardDTO.builder().bid(1L).description("testDescription")
                                .title("testTitle").time("2024-01-01 10:10:10").writer("tester1")
                                .build();
                UserDTO user = UserDTO.builder().id("testid1").uid(1L).name("tester1")
                                .password("1234").build();
                MockHttpSession httpSession = new MockHttpSession();
                when(boardService.getBoardById(1L)).thenReturn(board);
                when(boardService.isWriter(board, user)).thenReturn(true);
                httpSession.setAttribute("user", user);
                mockMvc.perform(get("/board/update/1").session(httpSession))
        ...
        void updatePostTest() throws Exception {
                BoardDTO changedBoard = BoardDTO.builder().bid(1L).description("changedDescription")
                                .title("changedTitle").time("2024-01-01 10:10:10").writer("tester1")
                                .build();
                UserDTO user = UserDTO.builder().id("testid1").uid(1L).name("tester1")
                                .password("1234").build();
                MockHttpSession httpSession = new MockHttpSession();
                ...
                mockMvc.perform(post("/board/update/1").session(httpSession)
                                .param("title", "changedTitle")
                                .param("description", "changedDescription")
                                .param("writer", "tester1"))
                                .andExpect(model().attribute("isWriter", true))
                                .andExpect(status().isOk())
        ...
        void deletePostTest() throws Exception {
                BoardDTO board = BoardDTO.builder().bid(1L).writer("testName").title("testTitle")
                                .description("testDescription").build();
                MockHttpSession session = new MockHttpSession();
                UserDTO user = UserDTO.builder().uid(1L).name("testName").id("testid")
                                .password("testPassword").build();
                session.setAttribute("user", user);
                when(boardService.getBoardById(1L)).thenReturn(board);
                when(boardService.isWriter(board, user)).thenReturn(true);

                mockMvc.perform(get("/board/delete/1").session(session))
